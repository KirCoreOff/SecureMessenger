...
namespace Messenger
{
    ...
    {
        private const int sizeInput = 64;
        public enum OutSize
        {
            bit256,
            bit512
        ...
        private static readonly byte[] nullBytes = Enumerable.Repeat(0x00, sizeInput).Select(x => (byte)x).ToArray();
        public static byte[] HashFunc(byte[] data, OutSize size, TypeInput typeInput)
        {
            if (typeInput == TypeInput.HexCodes)
                data = ConvertHexCodesToChars(data);

            int n = data.Length / sizeInput;
            byte[] h, N, Sigma;
            h = new byte[sizeInput];
            N = new byte[sizeInput];
            Sigma = new byte[sizeInput];
            if (size == OutSize.bit512)
            {
                nullBytes.CopyTo(h, 0);
            }
            else
            {
                h = Enumerable.Repeat(0x01, sizeInput).Select(x => (byte)x).ToArray();
            }
            byte[] m;
            //Обработка блоков по 512 бит
            for (int i = 1; i <= n; i++)
            {
                m = new byte[sizeInput];
                data.TakeLast(sizeInput).ToArray().CopyTo(m, 0);
                h = gFunc(N, h, m);
                byte[] numberInBytes = new byte[sizeInput];
                BitConverter.GetBytes(512).CopyTo(numberInBytes, 0);
                Array.Reverse(numberInBytes);
                N = AddMod512(N, numberInBytes);
                Sigma = AddMod512(Sigma, m);
                data = data.SkipLast(i * sizeInput).ToArray();
            }
            //Дополнение последнего блока до 512 бит
            m = new byte[sizeInput];
            m[sizeInput - data.Length - 1] = 1;
            Array.Copy(data, 0, m, sizeInput - data.Length, data.Length);
            //Обработка последнего блока
            h = gFunc(N, h, m);
            byte[] lengthInBytes = new byte[sizeInput];
            BitConverter.GetBytes(data.Length * 8).CopyTo(lengthInBytes, 0);
            Array.Reverse(lengthInBytes);
            N = AddMod512(N, lengthInBytes);
            Sigma = AddMod512(Sigma, m);
            h = gFunc(nullBytes, h, N);
            h = gFunc(nullBytes, h, Sigma);
            if (size == OutSize.bit512)
                return h;
            return h.Take(32).ToArray();
        }

        private static byte[] ConvertHexCodesToChars(byte[] data)
        {
            int n = data.Length / 2;
            for (int i = 0; i < n; i++)
            {
                data[i] = Convert.ToByte(((char)data[2 * i]).ToString() + (char)data[2 * i + 1], 16);
            }
            return data.Take(n).ToArray();
        }

        private static byte[] gFunc(byte[] N, byte[] h, byte[] m)
        {
            var K = XOR(h, N);
            K = SFunc(K);
            K = PFunc(K);
        ...
        {
            return first.Select((x, i) => (byte)(x ^ second[i])).ToArray();
        }

        private static byte[] SFunc(byte[] K)
        {
            return K.Select(x => Pi[x]).ToArray();
        }
        private static byte[] PFunc(byte[] K)
        {
            return K.Select((x, i) => K[Tau[i]]).ToArray();
        }
        private static byte[] LFunc(byte[] K)
        {
            byte[] result = new byte[sizeInput];
            for (int i = 0; i < 8; i++)
            {
                ...
                for (int j = 0; j < 8; j++)
                {
                    currentPart += (ulong)K[i * 8 + j] << (sizeInput - (j + 1) * 8); //Переводим 8 байт массива в длинное 8-байтовое целое
                }
                for (int j = 0; j < 64; j++)
                {
                    if ((currentPart >> j & 1) == 1)
                        currentResult ^= A[sizeInput - j - 1];
                }
                for (int j = 0; j < 8; j++)
                {
                    result[i * 8 + j] = (byte)(currentResult >> (sizeInput - (j + 1) * 8) & 0xFF); //Из 8-байтового целого в массив из 8 байт
                }
            }
            return result;
        }

        private static byte[] EFunc(byte[] K, byte[] m)
        {
            var state = XOR(K, m);
            for (int i = 0; i < 12; i++)
            {
